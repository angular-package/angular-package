import { ConsoleColor } from '../type/console-color.type';
import { ConsoleStyle } from '../type/console-style.type';

export class ConsoleClass {
  private colors = {
    default: '\x1b[0m',
    reset: '\x1b[0m',

    // text color
    black: '\x1b[30m',
    red: '\x1b[31m',
    green: '\x1b[32m',
    yellow: '\x1b[33m',
    blue: '\x1b[34m',
    magenta: '\x1b[35m',
    cyan: '\x1b[36m',
    white: '\x1b[37m',

    // background color
    defaultBg: '\x1b[39m',
    blackBg: '\x1b[40m',
    redBg: '\x1b[41m',
    greenBg: '\x1b[42m',
    yellowBg: '\x1b[43m',
    blueBg: '\x1b[44m',
    magentaBg: '\x1b[45m',
    cyanBg: '\x1b[46m',
    whiteBg: '\x1b[47m'
  };

  private styles = {
    bold: '\x1b[1m',
    reset: '\x1b[0m',
    faint: '\x1b[2m'
  };
  private text$$ = '';

  public blue(text: string, style?: ConsoleStyle): this {
    this.prepare(text, 'blue', style);

    return this;
  }

  public green(text: string, style?: ConsoleStyle): this {
    this.prepare(text, 'green', style);

    return this;
  }

  public info(): this {
    console.info(`${ this.text$$ }`);
    this.text$$ = '';

    return this;
  }

  public log(display: boolean = true): this {
    if (display === true) {
      console.log(`${this.styles.reset} ${ this.text$$ } ${this.styles.reset}`);
    }
    this.text$$ = '';

    return this;
  }

  public red(text: string, style?: ConsoleStyle): this {
    this.prepare(text, 'red', style);

    return this;
  }

  public text(text: string, color?: ConsoleColor, style?: ConsoleStyle): this {
    this.prepare(text, color, style);

    return this;
  }

  public yellow(text: string, style?: ConsoleStyle): this {
    this.prepare(text, 'yellow', style);

    return this;
  }

  public variable(name: string, value: any): this {
    console.log(`${this.styles.reset} ${name} ${this.styles.reset}`, value);

    return this;
  }


  private prepare(text: string, color: ConsoleColor = 'reset', style?: ConsoleStyle): string {
    let styles = '';
    if (style) {
      style.forEach((s: string) => {
        styles = `${styles}${this.styles[s]}`;
      });
    }

    return this.text$$ = `${this.text$$}${this.colors[color]}${styles}${text}${this.styles.reset}`;
  }
}